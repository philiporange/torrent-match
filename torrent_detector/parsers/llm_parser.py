...
class LLMParser(Parser):
    ...
    def _build_prompt(self, name: str, content: TorrentContent) -> str:
        """Build JSON-formatted prompt for LLM"""
        return f"""Parse the following torrent name and extract media information.
...
- "media_type": string - One of: "MOVIE", "TV_EPISODE", "TV_SEASON", "TV_MULTI_SEASON", or "UNKNOWN"
- "quality": string or null - Video quality (e.g., "1080p", "720p")
- "source": string or null - Source type (e.g., "BluRay", "WEB-DL")
- "codec": string or null - Video codec (e.g., "x264", "x265")
- "group": string or null - Release group name

Focus on accuracy. Use null for fields you're unsure about.
Respond with ONLY the JSON object, no additional text."""

    def _create_parse_result(self, result: dict) -> Optional[ParseResult]:
        """Create ParseResult from validated JSON response"""
        try:
            ...
            # Parse media type
            media_type_str = result.get("media_type", "UNKNOWN").upper()
            try:
                media_type = MediaType[media_type_str]
            except KeyError:
                vprint(f"Invalid media_type '{media_type_str}', defaulting to UNKNOWN")
